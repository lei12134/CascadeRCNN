...
class DeltaBoxCoder(object):
    def __init__(self,
                 targets=((0., 0., 0., 0.), (1., 1., 1., 1.)),
                 bbox_xform_clip=math.log(1000. / 16)):
        self.means = targets[0]
        self.stds = targets[1]
        self.bbox_xform_clip = bbox_xform_clip

    def encode(self, reference_boxes, proposals):
        # 统计每张图像的 anchors 个数，方便处理后再分开
        boxes_per_image = [len(b) for b in reference_boxes]
        reference_boxes = torch.cat(reference_boxes, dim=0)
        proposals = torch.cat(proposals, dim=0)
    ...
    def encode_single(self, reference_boxes, proposals):
        assert reference_boxes.shape == proposals.shape
        dtype = reference_boxes.dtype
        device = reference_boxes.device
        means = torch.as_tensor(self.means, dtype=dtype, device=device)
        stds = torch.as_tensor(self.stds, dtype=dtype, device=device)
        ...
        rx = (reference_boxes[:, 0] + reference_boxes[:, 2]) * 0.5
        ry = (reference_boxes[:, 1] + reference_boxes[:, 3]) * 0.5
        rw = reference_boxes[:, 2] - reference_boxes[:, 0]
        rh = reference_boxes[:, 3] - reference_boxes[:, 1]
    ...
    def decode(self, relcodes: Tensor, boxes: List[Tensor]):
        boxes = torch.cat(boxes, 0)  # (N, 4)
        relcodes = relcodes.reshape(-1, 4)  # (N, 4)
        assert relcodes.shape == boxes.shape
        decode_boxes = self.decode_single(relcodes, boxes)
        return decode_boxes

    def decode_single(self, relcodes, boxes):
        dtype = relcodes.dtype
        device = relcodes.device
        means = torch.as_tensor(self.means, dtype=dtype, device=device)
        stds = torch.as_tensor(self.stds, dtype=dtype, device=device)

        relcodes = relcodes * stds + means
        dx = relcodes[:, 0]
        dy = relcodes[:, 1]
        dw = relcodes[:, 2]
        dh = relcodes[:, 3]
        ...
        bx = (boxes[:, 0] + boxes[:, 2]) * 0.5
        by = (boxes[:, 1] + boxes[:, 3]) * 0.5
        bw = boxes[:, 2] - boxes[:, 0]
        bh = boxes[:, 3] - boxes[:, 1]
        ...
        bx2 = px + pw * 0.5
        by2 = py + ph * 0.5
        proposals = torch.stack((bx1, by1, bx2, by2), dim=1)
        return proposals
